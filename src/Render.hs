...
module Render (renderNote, renderTree, defaultStyle) where
...
renderNote [] = mempty
renderNote ((Tag tag, contents) : xs) =
    details_ [open_ "open"] (summary_ (toHtml tag) <> renderContent contents) <> renderNote xs
  where
